...
# declara valores. La correspondencia con el memoria (capitulo 3) se indica
# en cada bloque.
...
# Nota metodologica (memoria 3.2 y observacion de la autora): el ticker del
...
# Regla de conjunto comun (memoria 3.2): si un indice no cubre 2008 se retira
# del estudio COMPLETO, no solo de esa crisis. Afecta en particular al
...
# definida en el memoria. Con FALSE, el analisis se DETIENE y pide una
# decision expresa; con TRUE, retira los indices afectados y lo documenta.
PERMITIR_EXCLUSION_AUTOMATICA <- FALSE

# --- Fechas de descarga (memoria 3.2: 2007-2023) -----------------------
DESCARGA_INICIO <- "2007-01-01"
DESCARGA_FIN    <- "2023-12-31"

# --- Crisis y ventanas temporales (memoria 3.3; Tabla 3.3) -------------
...
FACTOR_ANUAL    <- 252L     # dias de negociacion para anualizar. Actua solo como
                            # factor de escala: el agrupamiento usa variables
                            # estandarizadas, por lo que no influye en los grupos.
COLS_VARIABLES  <- c("rent_anual", "volatilidad", "drawdown", "correlacion_media")

# --- Clustering y seleccion de k (memoria 3.8) -------------------------
...
# --- Evaluacion de las expectativas (memoria 3.7 y 3.9) --------------
# 2008 y 2022 se evaluan de forma descriptiva, observando la composicion
# estructural de cada grupo; la COVID, por la separabilidad de la particion
# y el reparto de los mercados entre grupos. No se fijan umbrales: la
# valoracion es descriptiva. El Rand ajustado se reserva para comparar
# particiones ENTRE crisis (analisis 3.11).

# --- Analisis complementarios (memoria 3.11) ---------------------------
# Reaccion y recuperacion (descriptivo): umbral de caida y porcentaje de
# recuperacion sobre el drawdown maximo de cada mercado.
UMBRAL_CAIDA        <- -0.10   # -10 % respecto al maximo alcanzado desde el pico de la crisis
